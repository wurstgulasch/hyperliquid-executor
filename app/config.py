...
class Config(BaseModel):
    ...
    env: str = Field(default="MAINNET", description="Environment: TESTNET or MAINNET")
    ...
    risk_percent: float = Field(default=0.05, ge=0.0001, le=0.1, description="Risk percentage per trade")
    ...
    def validate_webhook_secret(cls, v, info):
        if info.data.get('env') == "MAINNET" and not v:
